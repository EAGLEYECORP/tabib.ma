...
export async function POST(req:Request){
 ...
 const {data,error}=await s.rpc('verify_claimed_directory_doctor',{p_doctor_id:String(x.doctorId||''),p_notes:x.notes?String(x.notes):null})
 if(error)return NextResponse.json({error:'VERIFICATION_REJECTED',detail:error.message},{status:400}); return NextResponse.json({ok:Boolean(data)})
}
